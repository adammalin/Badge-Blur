...
cleanup_stale_server() {
  ...
  if kill -0 "$stale_pid" 2>/dev/null; then
    echo "An old Badge Blur local service could not be stopped."
    exit 1
  fi
  /bin/rm -f "$PID_FILE"
}
